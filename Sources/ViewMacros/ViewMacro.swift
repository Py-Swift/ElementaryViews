    ...
    public static func expansion(
        ...
        if needsFunctionView {
            ...
            decls.append(
                DeclSyntax(
                    """
                    \(raw: pubText)static func __applyContext(_ context: borrowing _ViewContext, to view: inout Self) {
            ...
            if !stateMembers.isEmpty {
                ...
                decls.append(
                    DeclSyntax(
                        """
                        \(raw: pubText)static func __initializeState(from view: borrowing Self) -> _ViewStateStorage {
                ...
                decls.append(
                    DeclSyntax(
                        """
                        \(raw: pubText)static func __restoreState(_ storage: _ViewStateStorage, in view: inout Self) {
                ...
                decls.append(
                    DeclSyntax(
                        """
                        \(raw: pubText)typealias __ViewState = Void
                        """
                    )
                )
            }
                ...
                try result.append(
                    ExtensionDeclSyntax(
                        ...
                        extension \(raw: type.trimmedDescription): __ViewEquatable {
                            \(raw: pubText)static func __arePropertiesEqual(a: Self, b: Self) -> Bool {
                                return true 
